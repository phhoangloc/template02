    ...
    return (
        <div className='flex justify-between h-12'>
            ...
            <div className="flex">
                {currentUser?.id ?
                    <IconDrop
                        icon={currentUser?.avata ? <Image src={process.env.ftp_url + "template2/" + currentUser.avata.name} fill className='object-cover cursor-pointer' alt='avata' /> : <PersonIcon className='!w-auto !h-full p-2 cursor-pointer' />}
                        data={[
                            {
                                title: "Profile",
